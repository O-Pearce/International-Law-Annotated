...
2. Extract to CSV:
   - ICJ judgments/advisory opinions (numbered paragraphs like `112.`):
     ```bash
     python3 extract_icj.py input.pdf --out instrument.csv
     python3 qc.py instrument.csv
     ```
...
4. Build the site:
   ```bash
   python3 build_site.py instrument.csv --title "Instrument Title" --slug instrument --outdir site
   ```
